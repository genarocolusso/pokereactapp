import { Image, Button, Text, Icon,  HStack, color, Box } from '@chakra-ui/react';
...
export const Addedpokemon = ({pokemon, index, teamView} : pokemonProps) => {
  ...
  const avatar = pokemon?.isShiny ? pokemon?.sprite[1] : pokemon?.sprite[0];
   ...
   if(pokemon && !teamView ) return (
    ...
    <Button bg={"#313030"} _active={{bg:"#242424"}} _hover={{bg:"#242424"}} h={'55px'} minW={"300px"} display={"flex"} justifyContent={"space-between"} px={6} onClick={()=>handleSetShowPokemon({...pokemon})}>
     <Text fontWeight={"normal"} textTransform={'capitalize'}> {pokemon.name} </Text>
      <HStack>
  ...
  return pokemon && teamView ? (
    ...
    alignItems={"center"}
    >
     <Text fontSize={16} textTransform={'capitalize'}> {pokemon.name} </Text>
      <HStack>
